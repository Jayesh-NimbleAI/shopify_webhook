...
async function getRemoteConfig() {
  try {
    ...
    console.log(
      "🛠️ DB Host:",
      process.env.DB_HOST ||
        "UNDEFINED - Warning: pg will default to localhost!"
    );
    ...
    console.error(
      "Critical: Could not retrieve secrets from AWS",
      error
    );
    if (process.env.DB_HOST && process.env.DB_USER && process.env.DB_PASSWORD) {
      console.warn("⚠️ Falling back to local environment variables for database configuration.");
      return {
        host: process.env.DB_HOST,
...
async function ensureDatabaseExists(config) {
  ...
  try {
    ...
    console.error(
      'Error checking/creating database:',
      error.message
    );
    throw error;
...
async function initializeSchema() {
  ...
  try {
    ...
    await client.query(`
      ...
      BEGIN
        ...
        IF NOT EXISTS (
          ...
          FROM information_schema.columns
          WHERE table_name = 'users'
          AND column_name = 'signup_mobile_number'
        ) THEN
          ALTER TABLE users
          ADD COLUMN signup_mobile_number VARCHAR(50);
        END IF;
        ...
        IF NOT EXISTS (
          ...
          FROM information_schema.columns
          WHERE table_name = 'users'
          AND column_name = 'waba_details'
        ) THEN
          ALTER TABLE users
          ADD COLUMN waba_details JSONB;
        END IF;
        ...
        IF NOT EXISTS (
          ...
          ALTER TABLE users
          ADD COLUMN members_limit INTEGER DEFAULT NULL;
        END IF;
    ...
    if (
      ownerIdType.rows.length > 0 &&
      ownerIdType.rows[0].data_type === 'uuid'
    ) {
    ...
    await client.query(`
      ...
      BEGIN
        ...
        IF NOT EXISTS (
          ...
          ALTER TABLE team_members
          ADD CONSTRAINT
          team_members_team_id_email_key
          UNIQUE (team_id, email);
    ...
    await client.query(`
      ...
      BEGIN
        ...
        IF NOT EXISTS (
          ...
          ALTER TABLE messages
          ADD COLUMN message_metadata JSONB
          DEFAULT '{}'::jsonb;
        END IF;
        ...
        IF NOT EXISTS (
          ...
          ALTER TABLE messages
          ADD CONSTRAINT messages_wamid_unique
          UNIQUE (wamid);
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS
      idx_audience_list_contacts_list_id
      ON audience_list_contacts(list_id)
    `);
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS
      idx_user_media_user_id
      ON user_media(user_id)
    `);

    await client.query(`
      CREATE INDEX IF NOT EXISTS
      idx_user_media_created_at
      ON user_media(created_at)
    `);
    ...
    await client.query(`
      ...
      BEGIN
        ...
        IF NOT EXISTS (
          ...
          ADD COLUMN meta_template_id VARCHAR(255);
      ...
      BEGIN
        ...
        IF NOT EXISTS (
          ...
          WHERE constraint_name =
            'templates_media_id_fkey'
        ) THEN

          ALTER TABLE templates
          ADD CONSTRAINT templates_media_id_fkey
          FOREIGN KEY (media_id)
          REFERENCES user_media(media_id)
          ON DELETE SET NULL;
